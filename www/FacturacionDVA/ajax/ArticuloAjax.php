<?php
	...
	switch ($_GET["op"]) {
		...
		case 'SaveOrUpdate':			
			...
			$impuesto = $_POST["intImpuesto"];

			if(move_uploaded_file($imagen, "../Files/Articulo/".$ruta)){

				if(empty($_POST["txtIdArticulo"])){
					
					if($objArticulo->Registrar($idcategoria, $idunidad_medida, $nombre, $descripcion, "Files/Articulo/".$ruta, $impuesto)){
						echo "Articulo Registrado";
					}else{
						echo "Articulo no ha podido ser registado aquì.";
					}
				}else{
				...
				if(empty($_POST["txtIdArticulo"])){
					...
					if($objArticulo->Registrar($idcategoria, $idunidad_medida, $nombre, $descripcion, $ruta_img, $impuesto)){
						echo "Articulo Registrado";
					}else{
