...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				...
				<xsl:choose>
					<xsl:when test="INSConsultaVirtualTipoOutput">
						<xsl:for-each select="INSConsultaVirtualTipoOutput">
							<xsl:call-template name="ConsultaTipoEnvio"/>
						</xsl:for-each>
					</xsl:when>
	...
	<xsl:template name="ConsultaTipoEnvio">
				...
				<xsl:when test="SOA_RESULT_PLACA/ENCABEZADO/CODRES = 9 or SOA_RESULT_PLACA/ENCABEZADO/CODRES = 0 or SOA_RESULT_PLACA/ENCABEZADO/CODRES = 7 or SOA_RESULT_PLACA/ENCABEZADO/CODRES = 6">
					...
					<xsl:choose>
						<xsl:when test="NumeroConsecutivo!= ''">
							<input type="hidden" name="Transaccion" value="INSConsultaVirtual"/>
								...
								<table align="center" class="tabla">
									<tr>
										...
										<td colSpan="3" class="detalle">
											<select class="campo" name="TIPOENVIO">
												<OPTION VALUE="">--- Seleccione ---</OPTION>
												<xsl:for-each select="TipoEnvios/Fila">
													<option value="{Codigo}">
							...
							<br/>
							<xsl:choose>
								<xsl:when test="TipoComprobante=101">
									<xsl:call-template name="BotonesContinuar"/>
								</xsl:when>
								<xsl:otherwise>
					...
					<xsl:choose>
						<xsl:when test="NumeroConsecutivo!= ''">
							...
							<xsl:if test="SOA_RESULT_PLACA/ENCABEZADO/CODRES=0">
								...
								<xsl:if test="TIPOENVIO=2">
									<input type="hidden" name="NUMEROCAJAVIRTUAL" value="{Cajas/Fila/NumeroCaja}"/>
								</xsl:if>
							</xsl:if>
	...
	<xsl:template name="ConsultaVirtualConfirmar">
		<table class="tabla" align="center">
			<tr class="etiqueta">
				...
				<td class="etiqueta">
					<xsl:value-of select="NumeroCajaVirtual"/>/<xsl:value-of select="DescripcionCaja"/>
				</td>
			</tr>
				...
				<xsl:when test="SOA_RESULT_PLACA/ENCABEZADO/CODRES = 9 or SOA_RESULT_PLACA/ENCABEZADO/CODRES = 0 or SOA_RESULT_PLACA/ENCABEZADO/CODRES = 7 or SOA_RESULT_PLACA/ENCABEZADO/CODRES = 6">
					...
					<xsl:choose>
						<xsl:when test="NumeroConsecutivo!= ''">
							...
							<input type="hidden" name="DescripcionCaja" value="{DescripcionCaja}"/>
							<!-- -->
	...
	<xsl:template name="BotonesContinuar">
		<div align="center">
			<xsl:if test="SOA_RESULT_PLACA/ENCABEZADO/CODRES=0">
				<input name="Consultar" class="button_red" type="button" value="Continuar" onclick="if (document.forms[0].TIPOENVIO.value == '') alert('Seleccione el Tipo de Entrega. Por Favor!'); else submit();"/>
			</xsl:if>
			<input name="OtraConsulta" class="button_gris" type="button" value="Otra consulta" onclick="cargar(this,'Transaccion.aspx?Transaccion=ConsultaClasePlaca&amp;Filtro=~/XML/virtual/INSConsultarVirtual.xsl','Transaccion')"/>
		</div>
	</xsl:template>
	<xsl:template name="BotonesSITE">
		<div align="center">
			<input name="BotonPagar" class="button_red" type="button" value="   Pagar SIN RTV  " onclick="ValidarMarchamo()"/>
			<input name="OtraConsulta" class="button_gris" type="button" value="Otra consulta" onclick="cargar(this,'Transaccion.aspx?Transaccion=ConsultaClasePlaca&amp;Filtro=~/XML/virtual/INSConsultarVirtual.xsl','Transaccion')"/>
		</div>
	</xsl:template>
	<xsl:template name="BotonesTMP">
		<div align="center">
			<input name="BotonPagar" class="button_red" type="button" value="   Pagar Temporal  " onclick="ValidarMarchamo()"/>
			<input name="OtraConsulta" class="button_gris" type="button" value="Otra consulta" onclick="cargar(this,'Transaccion.aspx?Transaccion=ConsultaClasePlaca&amp;Filtro=~/XML/virtual/INSConsultarVirtual.xsl','Transaccion')"/>
		</div>
	</xsl:template>
