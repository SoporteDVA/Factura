...
<xsl:stylesheet xmlns:a="http://tempuri.org/" exclude-result-prefixes="a soap" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>Bitacora de Pagos Fecha y Oficina</title>
			...
			<xsl:choose>
				<xsl:when test="Consulta_Pagos_ResumenOutput">
					<xsl:for-each select="Consulta_Pagos_ResumenOutput">
				...
				<xsl:when test="Consulta_Pagos_ResumenFault">
					<xsl:for-each select="Consulta_Pagos_ResumenFault">
	...
	<xsl:template name="Consulta">
		<form action="Transaccion.aspx" method="post">
			<input type="hidden" name="Transaccion" value="Consulta_Pagos_Resumen"/>
			<input type="hidden" name="ServidorSTX" value="infoware.STX.Cajas"/>
			<input type="hidden" name="EstadoPagos" value="PG"/>
			<input type="hidden" name="Filtro" value="XML/WEBConsultas/ParametrosNGResumenFechaOficinaGrupo.xsl"/>
			<div align="center">
				<table align="center">
					<tr>
						<td width="30%">
			...
			<table align="center" width="500" border="0">
				<tr>
					<td width="50%" align="center">
						<font class="tituloAzulClaro">Consulta de Pagos Resumen por Oficina y Servicio</font>
		...
		<div align="center">
			<table align="center">
				<tr>
					...
					<td>
						<fONT class="tituloAzulClaro">CONSULTA RESUMIDA DE RECAUDACION</fONT>
					</td>
				</tr>
			</table>
		</div>
		...
		<xsl:for-each select="Resultado">
			<div align="right" width="100%">
				...
				<table class="tabla">
					...
					<tbody>
						<td class="detalle" align="right"><xsl:value-of select="Fila/FechaInicial"/></td>
						<td class="detalle" align="right"><xsl:value-of select="Fila/FechaFinal"/></td>
				...
				<tbody>
					...
					<xsl:for-each select="Fila">
						<tr>
							...
							<td align="center">
								<xsl:value-of select="Dia"/>/<xsl:value-of select="Mes"/>/<xsl:value-of select="Anno"/>
							</td>
						</tr>
					</xsl:for-each>
					<tr>
