...
<xsl:stylesheet xmlns:a="http://tempuri.org/" exclude-result-prefixes="a soap" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>Afiliados por Fecha y Servicio</title>
			...
			<xsl:choose>
				<xsl:when test="Consulta_AfiliadosOutput">
					<xsl:for-each select="Consulta_AfiliadosOutput">
				...
				<xsl:when test="Consulta_AfiliadosFault">
					<xsl:for-each select="Consulta_AfiliadosFault">
	...
	<xsl:template name="Consulta">
		<form action="Transaccion.aspx" method="post" target="_blank">
			<input type="hidden" name="Transaccion" value="Consulta_Afiliados"/>
			...
			<table align="center" width="500" border="0">
				<tr>
					<td width="50%" align="center">
						<font class="tituloAzulClaro">Consulta de Afiliados por Servicio</font>
		...
		<div align="center">
			<table align="center">
				<tr>
					...
					<td>
						<fONT class="tituloAzulClaro">CONSULTA DETALLADA AFILIADOS</fONT>
					...
					<xsl:for-each select="Fila">
						<tr>
							...
							<xsl:choose>
								...
								<xsl:when test="Estado='Inactiva' or Estado='PE' or Estado='SVE'">
							...
							<td align="justify">
								<xsl:value-of select="CuentaPrimaria"/>
							</td>
							<td align="left">
								<xsl:value-of select="CuentaSecundaria"/>
							</td>
							<td>
							...
							<td align="justify">
								<xsl:value-of select="UltimoProcesamiento"/>
							</td>
						</tr>
					</xsl:for-each>
					<!--<tr>
