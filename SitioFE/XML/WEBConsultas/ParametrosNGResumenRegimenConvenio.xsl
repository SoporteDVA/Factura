...
<xsl:stylesheet xmlns:a="http://tempuri.org/" exclude-result-prefixes="a soap" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			...
			<xsl:choose>
				<xsl:when test="Consulta_Pagos_RegimenCCSSOutput">
					<xsl:for-each select="Consulta_Pagos_RegimenCCSSOutput">
				...
				<xsl:when test="Consulta_Pagos_RegimenCCSSFault">
					<xsl:for-each select="Consulta_Pagos_RegimenCCSSFault">
	...
	<xsl:template name="Consulta">
		<form action="Transaccion.aspx" method="post">
			<input type="hidden" name="Transaccion" value="Consulta_Pagos_RegimenCCSS"/>
			<input type="hidden" name="ServidorSTX" value="infoware.STX.Cajas"/>
			<input type="hidden" name="Filtro" value="XML/WEBConsultas/ParametrosNGResumenRegimenConvenio.xsl"/>
			<div align="center">
				<table align="center">
					<tr>
						<td width="30%">
			...
			<table align="center" width="500" border="0">
				<tr>
					<td width="50%" align="center">
						<font class="tituloAzulClaro">Consulta Detalle de Pagos Convenio y Regimen (CCSS)</font>
		...
		<div align="center">
			<table align="center">
				<tr>
					...
					<td>
						<fONT class="tituloAzulClaro">CONSULTA DETALLADA RECAUDACION POR CONVENIO Y REGIMEN (CCSS)</fONT>
					</td>
				</tr>
			</table>
		</div>
			...
			<div align="right">
				...
				<table class="tabla">
					...
					<thead>
						<tr class="encabezado">
							<th>Convenio</th>
							<th>Monto Total IVM</th>
							<th>Monto Total SEM</th>
							<th>Monto Total</th>
					...
					<tbody>
						...
						<td class="tablaSinImprimir" align="right">¢<xsl:value-of select="format-number(sum(Resultado/Fila/MontoIVM),'###,###,###,##0.00')"/></td>
						<td class="tablaSinImprimir" align="right">¢<xsl:value-of select="format-number(sum(Resultado/Fila/MontoSEM),'###,###,###,##0.00')"/></td>
				...
				<tbody>
					...
					<xsl:for-each select="Resultado/Fila">
						<tr>
							...
							<td align="right">
								<xsl:choose>
									<xsl:when test="MontoIVM =''">0.00</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="format-number(MontoIVM,'###,###,###,##0.00')"/>
									</xsl:otherwise>
								</xsl:choose>
							</td>
							<td align="right">
								<xsl:choose>
									<xsl:when test="MontoSEM =''">0.00</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="format-number(MontoSEM,'###,###,###,##0.00')"/>
									</xsl:otherwise>
								</xsl:choose>
							</td>
							<td align="justify">
					...
					<tr>
						...
						<td class="etiqueta" align="right">
							<xsl:value-of select="format-number(sum(Resultado/Fila/MontoIVM),'###,###,###,##0.00')"/>
						</td>
						<td class="etiqueta" align="right">

							<xsl:value-of select="format-number(sum(Resultado/Fila/MontoSEM),'###,###,###,##0.00')"/>
						</td>
						<td class="etiqueta"/>
						<td class="etiqueta"/>
						<td class="etiqueta"/>
