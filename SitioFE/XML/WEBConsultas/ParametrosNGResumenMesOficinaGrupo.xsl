...
<xsl:stylesheet xmlns:a="http://tempuri.org/" exclude-result-prefixes="a soap" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>Bitacora de Pagos Resumen Mensual</title>
			...
			<xsl:choose>
				<xsl:when test="Consulta_NCResumenOutput">
					<xsl:for-each select="Consulta_NCResumenOutput">
				...
				<xsl:when test="Consulta_NCResumenFault">
					<xsl:for-each select="Consulta_NCResumenFault">
	...
	<xsl:template name="Consulta">
		<form action="Transaccion.aspx" method="post">
			<input type="hidden" name="Transaccion" value="Consulta_NCResumen"/>
			<input type="hidden" name="ServidorSTX" value="infoware.STX.Cajas"/>
			<input type="hidden" name="Filtro" value="XML/WEBConsultas/ParametrosNGResumenMesOficinaGrupo.xsl"/>
			<div align="center">
				<table align="center">
					<tr>
						<td width="30%">
			...
			<table align="center" width="500" border="0">
				<tr>
					<td width="50%" align="center">
						<font class="tituloAzulClaro">Consulta Resumen Mensual por Servicio (Nota Credito)</font>
		...
		<div align="center">
			<table align="center">
				<tr>
					...
					<td>
						<fONT class="tituloAzulClaro">CONSULTA RESUMIDA DE RECAUDACION NOTA CREDITO</fONT>
					</td>
				</tr>
			</table>
		</div>
			...
			<div align="right" width="100%">
				...
				<table class="tabla">
					...
					<tbody>
						...
						<td class="tablaSinImprimir" align="right">¢<xsl:value-of select="format-number(sum(Fila/Comision),'###,###,###,##0.00')"/></td>
						<td class="tablaSinImprimir" align="right">¢<xsl:value-of select="format-number((sum(Fila/Monto)-sum(Fila/Comision)),'###,###,###,##0.00')"/></td>
				...
				<tbody>
					...
					<tr>
						...
						<td class="etiqueta" align="right">
							<xsl:value-of select="format-number(sum(Fila/Monto),'###,###,###,##0.00')"/>
						</td>
						<td class="etiqueta" align="right">
