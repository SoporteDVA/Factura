...
<xsl:stylesheet xmlns:a="http://tempuri.org/" exclude-result-prefixes="a soap" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>Bitacora de Notas Credito por Servicio</title>
			...
			<xsl:choose>
				<xsl:when test="Consulta_NotasServicioGruposOutput">
					<xsl:for-each select="Consulta_NotasServicioGruposOutput">
				...
				<xsl:when test="Consulta_NotasServicioGruposFault">
					<xsl:for-each select="Consulta_NotasServicioGruposFault">
	...
	<xsl:template name="Consulta">
		<form action="Transaccion.aspx" method="post">
			<input type="hidden" name="Transaccion" value="Consulta_NotasServicioGrupos"/>
			<input type="hidden" name="ServidorSTX" value="infoware.STX.Cajas"/>
			<input type="hidden" name="Filtro" value="XML/WEBConsultas/ParametrosNGResumenNC.xsl"/>
			<div align="center">
				<table align="center">
					<tr>
						<td width="30%">
			...
			<table align="center" width="500" border="0">
				<tr>
					<td width="50%" align="center">
						<font class="tituloAzulClaro">Consulta Notas de Credito por Colector (Servicio)</font>
				...
				<tr>
					...
					<td colSpan="3" class="detalle">
						<select class="campo" name="Colector">
							<option value="1000" selected="selected">--TODAS--</option>
		...
		<div align="center">
			<table align="center">
				<tr>
					...
					<td>
						<fONT class="tituloAzulClaro">CONSULTA DE NOTAS DE CREDITO POR SERVICIO</fONT>
					</td>
				</tr>
			</table>
		</div>

		<xsl:for-each select="Resultados[Resultado/Fila/Monto!='']">
				...
				<table class="tabla">
					...
					<tbody>
						<td class="tablaSinImprimir" align="center">
							<xsl:value-of select="Resultado/Fila/NombreServicio"/>
						</td>
						<td class="tablaSinImprimir" align="right">¢<xsl:value-of select="format-number(sum(Resultado/Fila/Monto),'###,###,###,##0.00')"/></td>
						<td class="tablaSinImprimir" align="right">¢<xsl:value-of select="format-number(sum(Resultado/Fila/Comision),'###,###,###,##0.00')"/></td>
						<td class="tablaSinImprimir" align="right">¢<xsl:value-of select="format-number((sum(Resultado/Fila/Monto)-sum(Resultado/Fila/Comision)),'###,###,###,##0.00')"/></td>
						<td class="tablaSinImprimir" align="center">
							<xsl:value-of select="format-number(sum(Resultado/Fila/Cantidad),'#')"/>
						</td>
					</tbody>
				</table>
				<br/>
				<input class="button" type="button" onclick="tableToExcel('{Resultado/Fila/Servicio}', '{Resultado/Fila/Servicio}')" value="Exportar a Excel:Servicio {Resultado/Fila/NombreServicio}"/>
			</div>
			<br/>
			<table id="{Resultado/Fila/Servicio}" class="tabla" align="right" width="100%">
				<thead>
					<tr class="etiqueta">
						<th align="center">Servicio-Colector</th>
						<th>Monto</th>
						<th>Comision</th>
						<th>Cantidad</th>
						<th>NotaCredito</th>
				...
				<tbody>
					...
					<xsl:for-each select="Resultado/Fila">
						<tr>
							...
							<td align="left">
								...
								<xsl:value-of select="NombreServicio"/>
							</td>
							<td align="right">
								<xsl:choose>
					...
					<tr>
						...
						<td class="etiqueta" align="right">
							<xsl:value-of select="format-number(sum(Resultado/Fila/Monto),'###,###,###,##0.00')"/>
						</td>
						<td class="etiqueta" align="right">

							<xsl:value-of select="format-number(sum(Resultado/Fila/Comision),'###,###,###,##0.00')"/>
						</td>
