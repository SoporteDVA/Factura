...
<xsl:stylesheet xmlns:a="http://tempuri.org/" exclude-result-prefixes="a soap" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>Bitacora de Pagos Oficina Actual</title>
			...
			<xsl:choose>
				<xsl:when test="Consulta_Pagos_OficinaOutput">
					<xsl:for-each select="Consulta_Pagos_OficinaOutput">
				...
				<xsl:when test="Consulta_Pagos_OficinaFault">
					<xsl:for-each select="Consulta_Pagos_OficinaFault">
	...
	<xsl:template name="ConsultaResumida">
		<form action="Transaccion.aspx" method="post">
			<input type="hidden" name="Transaccion" value="Consulta_Pagos_Oficina"/>
			<input type="hidden" name="ServidorSTX" value="infoware.STX.Cajas"/>
			<input type="hidden" name="Filtro" value="XML/WEBConsultas/ParametrosNGResumenOficinaActual.xsl"/>
			<div align="center">
				<table align="center">
					<tr>
						<td width="30%">
			...
			<table align="center" width="500" border="0">
				<tr>
					<td width="50%" align="center">
						<font class="tituloAzulClaro">Consulta de Pagos Oficina Actual (Resumen)</font>
						<br/>
	...
	<xsl:template name="Consulta">
		<form action="Transaccion.aspx" method="post">
			...
			<table align="center" width="500" border="0">
				<tr>
					<td width="50%" align="center">
						<font class="tituloAzulClaro">Consulta de Pagos Oficina Actual</font>
						<br/>
		...
		<div align="center">
			<table align="center">
				<tr>
					...
					<td>
						<fONT class="tituloAzulClaro">CONSULTA DETALLADA DE PAGOS SUCURSAL</fONT>
						<br/>

						<font class="tituloAzulClaro">Usuario Actual : <xsl:value-of select="Resultado/Sesion/CajaUsuario"/></font>
						<br/>
						<font class="tituloAzulClaro">Oficina Actual : <xsl:value-of select="Resultado/Fila/Oficina"/>-<xsl:value-of select="Resultado/Fila/NombreOficina"/></font>
					</td>
				</tr>
			</table>
		</div>
							...
							<td align="center">
								<xsl:choose>
									<xsl:when test="Estado='PG'">
										<p align="center">
											<a>
												<xsl:attribute name="href">Transaccion.aspx?Transaccion=xp_BitacoraCJPago&amp;Colector=<xsl:value-of select="Servicio"/>&amp;Convenio=<xsl:value-of select="Convenio"/>&amp;Numero=<xsl:value-of select="Numero"/>&amp;Documento=<xsl:value-of select="NumeroDocumento"/>&amp;FechaConsulta=<xsl:value-of select="Fecha"/>&amp;TipoEmision=Duplicado&amp;ServidorSTX=infoware.STX.Cajas&amp;Filtro=XML/WEBImpresion/WEBImpresion.xsl&amp;onclick="window.open('', 'v', 'width=300,height=200');</xsl:attribute><img hspace="1" src="imagenes/vwicn032.gif" align="center" border="0" alt="Impresion" /></a>
										</p>
									</xsl:when>
									<xsl:otherwise>Sin Proceso</xsl:otherwise>
								</xsl:choose>
