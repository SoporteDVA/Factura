...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="BPD/Version='1.0.0.0'">
				<Process name="{BPD/Name}" BPMversion="1.0.0.0">
					<partnerLinks>
			...
			<xsl:otherwise>
			Versión(<xsl:value-of select="BPD/Version"/>) incompatible con la plantilla BPM2BPEL.
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="secuencia">
		...
		<xsl:for-each select="/BPD/Pools/Pool/Process/GraphicalElements/SequenceFlow[@Source=$idRight or @Source=$idBottom]">
			<xsl:if test="Assignments">
				...
				<xsl:variable name="sigActividad" select="../*[@IdLeft=$target or @IdTop=$target]"/>
				<assign>
						...
						<copy>
							...
							<to>
								<xsl:attribute name="variable">
									<xsl:for-each select="$sigActividad">
										<xsl:if test="name()!='End'"><xsl:value-of select="$sigActividad/Name"/>Input</xsl:if>
										<xsl:if test="name()='End' and EndResult='Message'"><xsl:value-of select="/BPD/Name"/>Output</xsl:if>
										<xsl:if test="name()='End' and EndResult='Error'"><xsl:value-of select="/BPD/Name"/>Fault</xsl:if>
									</xsl:for-each>/<xsl:value-of select="@to"/></xsl:attribute>
							</to>
						</copy>
					</xsl:for-each>
				</assign>
	...
	<xsl:template name="buscarActividad">
		<xsl:param name="id"/>
		<xsl:for-each select="/BPD/Pools/Pool/Process/GraphicalElements/*[@IdLeft=$id or @IdTop=$id]">
			<xsl:choose>
					...
					<xsl:choose>
						...
						<xsl:when test="name()='End'">
							<xsl:if test="EndResult='Message'">
								<reply variable="{concat(/BPD/Name,'Output')}"/>
							</xsl:if>
							<xsl:if test="EndResult='Error'">
								<throw faultVariable="{concat(/BPD/Name,'Fault')}"/>
							</xsl:if>
						</xsl:when>
					</xsl:choose>
					<xsl:call-template name="secuencia">
		...
		<xsl:for-each select="Assignments">
			<assign>
				<xsl:for-each select="Assignment">
					<copy>
						<from type="{@type}" value="{@from}"/>
						<to variable="{concat(../../Name, 'Input/',@to)}"/>
					</copy>
				</xsl:for-each>
			</assign>
		</xsl:for-each>
		...
		<xsl:choose>
			<xsl:when test="parent::Message">
				<xsl:choose>
					...
					<xsl:when test="../../EndResult='Message'">
						<xsl:element name="{concat(/BPD/Name,'Output')}">
							<xsl:apply-templates/>
						</xsl:element>
					</xsl:when>
					<xsl:when test="../../EndResult='Error'">
						<xsl:element name="{concat(/BPD/Name,'Fault')}">
							<xsl:apply-templates/>
						</xsl:element>
					</xsl:when>
