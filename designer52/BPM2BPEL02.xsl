...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="BPD/Version='1.0.0.2'">
				<Process name="{BPD/Name}" BPMversion="1.0.0.2">
					<partnerLinks>
			...
			<xsl:otherwise>Versión(<xsl:value-of select="BPD/Version"/>) incompatible con la plantilla BPM2BPEL02.</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- Funcciones para procesar Actividades -->
	<xsl:template name="main">
    <!-- Se agregaron las condiciones llave del proceso -->
      <Keys>
        <xsl:value-of select="BPD/Pools/Pool/Process/Keys"/>
      </Keys>
		<xsl:for-each select="BPD/Pools/Pool/Process/GraphicalElements">
			<variables>
        <!-- Se agregaron las propiedades del proceso-->
        <xsl:element name="Globales">
          <xsl:for-each select="/BPD/Pools/Pool/Process/Properties">
			...
			<sequence>
				<receive variable="{concat(/BPD/Name,'Input')}" id="{Start/@ID}"/>
		...
		<xsl:for-each select="/BPD/Pools/Pool/Process/GraphicalElements/SequenceFlow[@Source=$idRight or @Source=$idBottom]">
			<xsl:if test="Assignments">
              ...
              <from type="{@type}" value="{@from}">
                <xsl:copy-of select="xsl/xsl:stylesheet"/>
              </from>
							<to variable="{@to}"/>
						</copy>
					</xsl:for-each>
				...
				<xsl:otherwise>
					<xsl:choose>
						...
						<xsl:when test="name()='Transformation'">
							<transformation name="{Name}" variable="{concat(Name,'Output')}" id="{@ID}">
								<xsl:copy-of select="xsl:stylesheet"/>
							</transformation>
						</xsl:when>
						<xsl:when test="name()='Converter'">
							<convert name="{Name}" inputVariable="{concat(Name,'Input')}" outputVariable="{concat(Name,'Output')}" type="{@type}" id="{@ID}"/>
						</xsl:when>
						<xsl:when test="name()='End'">
							<xsl:if test="EndResult='Message'">
								<reply variable="{OutputName}" id="{@ID}"/>
							</xsl:if>
							<xsl:if test="EndResult='Error'">
								<throw faultVariable="{OutputName}" id="{@ID}"/>
							</xsl:if>
						</xsl:when>
					</xsl:choose>
					<xsl:call-template name="secuencia">
	...
	<xsl:template name="invoke_scope">
		...
		<xsl:choose>
			...
			<xsl:when test="@loopType='Standard'">
				<while testTime="{@testTime}" condition="{@miCondition}" loopCounter="{@loopCounter}" id="{@ID}">
			...
			<xsl:when test="@loopType='MultiInstance'">
				<foreach variable="{@miCondition}" id="{@ID}">
					<xsl:call-template name="invoke_scope_inner">
						<xsl:with-param name="inner" select="$inner"/>
						<xsl:with-param name="assign" select="@assignTime"/>
					</xsl:call-template>
	...
	<xsl:template name="invoke_scope_inner">
		...
		<xsl:element name="{$inner}">
			...
			<xsl:attribute name="NombreProceso">
				<xsl:value-of select="@NombreProceso"/>
			</xsl:attribute>
			<xsl:choose>
...
<metaInformation>
<scenarios ><scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="Prueba.xml" htmlbaseurl="" outputurl="" processortype="internal" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext=""/></scenarios><MapperMetaTag><MapperInfo srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/><MapperBlockPosition></MapperBlockPosition></MapperMetaTag>
</metaInformation>
-->
